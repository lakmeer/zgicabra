...
fn draw_swarm_panel (y: u16, swarm: &SwarmView) {
    draw_range_label(2, y + 1, "chase",  swarm.chase_factor.value(), 0.5,  1.0);
    draw_range_label(2, y + 2, "radius", swarm.radius.value(),       0.0,  200.0);
    draw_range_label(2, y + 3, "orbit",  swarm.orbit_speed.value(),  0.0,  2.0);
    draw_range_label(2, y + 4, "phaser", swarm.phaser_depth.value(), 0.0,  1.0);
    draw_range_label(2, y + 5, "xover",  swarm.xover_freq.value(),   0.0,  2000.0);
    ...
    line(&mut canvas, 0.0, px_h / 2.0, px_w - 1.0, px_h / 2.0, PixelColor::BrightBlack);
    line(&mut canvas, px_w / 2.0, 0.0, px_w / 2.0, px_h - 1.0, PixelColor::BrightBlack);
    ...
    drawille_paste(&mut rows, SWARM_SCOPE_X, y + 1);

    print!("{}{:<13}{:>7.1} Hz  {:<13}{:>6.0} cents",
        goto(SWARM_SCOPE_X, y + SWARM_SCOPE_ROWS as u16 + 2),
        "origin", origin,
        "radius", swarm.radius.value());
}
